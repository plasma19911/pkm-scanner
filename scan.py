...
def write_app_data(all_offers, all_preorders, all_alerts, scan_time,
                   errors, shop_count, changes=None, out_dir="."):
    """Schreibt data.json - die Datenquelle der Handy-App (app.html).

    Bewusst schlank gehalten: nur Angebote, Vorbestellungen und
    Alarm-Treffer, jeweils schon nach Produkt gruppiert. Die App rendert
    daraus selbst, statt fertiges HTML zu laden."""
    changes = changes or {}
    payload = {
        "version": 1,
        "scan_time": scan_time,
        ...
        "alerts": [
            {"keyword": kw, "items": _app_groups_from(hits, "alert")}
            for kw, hits in sorted(all_alerts.items())
            if hits
        ],
        "changes": {
...
def publish_static(out_dir):
    ...
    buckets += [a.get("items", []) for a in data.get("alerts", [])]
...
def run_scan():
    ...
    write_app_data(all_offers, all_preorders, all_alerts, scan_time,
